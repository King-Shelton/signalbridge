...
export default function SignalRadarPage() {
  ...
  const channels = useMemo(() => Array.from(new Set(items.map((i) => i.channel))), [items]);
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div>
        ...
        <p className="mt-1 text-[13px] text-[rgba(214,235,230,0.5)]">Explainable priority signals — each card shows why a youth is flagged.</p>
      </div>
        ...
        <div className="flex gap-1 glass-card p-1">
          {RISK_FILTERS.map((r) => (
            <button
              ...
              style={{
                background: risk === r ? "rgba(31,111,100,0.25)" : "transparent",
                color: risk === r ? "#6fb8aa" : "rgba(214,235,230,0.5)",
                border: risk === r ? "1px solid rgba(111,184,170,0.3)" : "1px solid transparent",
              }}
            >
              {r === "all" ? "All risks" : r}
            </button>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {filtered.map((item) => (
          ...
          >
            <div className="flex items-start justify-between gap-3 mb-4">
              <div>
                <Link href={`/worker/youths/${item.youthId}`} className="text-[16px] font-semibold text-[#f1f6f4] hover:text-[#6fb8aa] transition-colors" style={{ letterSpacing: "-0.01em" }}>
                  {item.youthName}
                </Link>
            ...
            <ul className="space-y-2 mb-4">
              {item.signals.map((s) => (
                <li key={s.id} className="p-3 rounded-[12px]" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <span className="text-[12px] font-semibold text-[rgba(214,235,230,0.8)]">{label(s.type)}</span>
                    <span className="text-[10.5px] font-mono text-[rgba(214,235,230,0.3)]">{s.source}</span>
                  </div>
                  <p className="text-[12px] text-[rgba(214,235,230,0.55)] leading-relaxed">{s.reason}</p>
                </li>
              ))}
            </ul>
            ...
            <Link
              ...
              className="text-[12.5px] font-semibold text-[#6fb8aa] hover:text-[#8fcfc3] transition-colors"
            >
              Open memory card →
            </Link>
          </article>
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="col-span-2 glass-card p-8 text-center">
            ...
            <button onClick={() => { setRisk("all"); setChannel("all"); setQuery(""); }} className="mt-3 text-[12.5px] text-[#6fb8aa] underline">
              Clear filters
            </button>
          </div>
        )}
