// forked from design-sync lib/bundle.mjs — alias next/link and next/navigation to stubs so previews render outside Next.js
...
const __dirname_override = dirname(fileURLToPath(import.meta.url));
// Stubs live in .ds-sync/stubs/ (staged alongside the converter scripts).
const stubsDir = join(__dirname_override, '../../.ds-sync/stubs');

// Redirect Next.js imports to lightweight stubs so component previews
// render in headless Chromium without a Next.js app router context.
const nextStubsPlugin = {
  name: 'next-stubs',
  setup(b) {
    b.onResolve({ filter: /^next\/link$/ }, () => ({
      path: join(stubsDir, 'next-link.mjs'),
    }));
    b.onResolve({ filter: /^next\/navigation$/ }, () => ({
      path: join(stubsDir, 'next-navigation.mjs'),
    }));
  },
};
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  const plugins = [nextStubsPlugin, reactShim];
  ...
  return {
    ...
    minify: false,
    define: { 'process.env.NODE_ENV': '"development"', 'process.env': '{"NODE_ENV":"development"}' },
    // lucide-react (and other packages) reference `process` which doesn't
    // exist in a browser IIFE. Shim it via banner so the IIFE runs cleanly.
    banner: { js: 'var process=typeof process!=="undefined"?process:{env:{NODE_ENV:"development"}};' },
  };
}
